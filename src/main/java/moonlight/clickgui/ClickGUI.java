...
import moonlight.utils.MouseUtils;
...
public class ClickGUI extends GuiScreen {
	...
	public void initGui() {	
		if(Moonlight.instance.fileManager != null) {
			Moonlight.instance.fileManager.getPositionManager().save();
		}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    	...
    	for(Window window : Moonlight.instance.windowManager.getWindows()) {
    		...
    		if(window.isDrag() && window.isDragging() && (Moonlight.instance.moduleManager.getModuleByName("HUD").isToggled() && Moonlight.instance.settingsManager.getSettingByName(Moonlight.instance.moduleManager.getModuleByName("HUD"), window.getName()).getValBoolean())) {
    ...
    protected void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) throws IOException {
        ...
        for(Window window : Moonlight.instance.windowManager.getWindows()) {
        	if(MouseUtils.isInside(mouseX, mouseY, window.getX(), window.isShowWindow() ? window.getY() - 14 : window.getY(), window.getWidth(), window.isShowWindow() ? window.getY() : window.getHeight()) && (Moonlight.instance.moduleManager.getModuleByName("HUD").isToggled() && Moonlight.instance.settingsManager.getSettingByName(Moonlight.instance.moduleManager.getModuleByName("HUD"), window.getName()).getValBoolean()) && mouseButton == 0) {
