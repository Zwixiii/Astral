...
public class ConfigManager {
	...
	public ConfigManager() {
		...
		configDir = new File(mc.mcDataDir, "moonlight/config");
	...
	public void save() {
		...
		for (Module mod : Moonlight.instance.moduleManager.modules) {
			toSave.add("MOD:" + mod.getName() + ":" + mod.isToggled() + ":" + mod.getKey());
		}
		
		for (Setting set : Moonlight.instance.settingsManager.getSettings()) {
			if (set.isCheck()) {
	...
	public void load() {
		...
		for (String s : lines) {
			...
			if (s.toLowerCase().startsWith("mod:")) {
				Module m = Moonlight.instance.moduleManager.getModuleByName(args[1]);
				if (m != null) {
					m.setToggled(Boolean.parseBoolean(args[2]));
					m.setKey(Integer.parseInt(args[3]));
				}
			} else if (s.toLowerCase().startsWith("set:")) {
				Module m = Moonlight.instance.moduleManager.getModuleByName(args[2]);
				if (m != null) {
					Setting set = Moonlight.instance.settingsManager.getSettingByName(m, args[1]);
					if (set != null) {
						if (set.isCheck()) {
